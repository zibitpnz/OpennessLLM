...
Актуальная версия и схемы перечислены в
[README](README.md#current-version-and-compatibility). Если номера расходятся,
см. раздел 29 ниже; ошибки истории completion описаны в разделе 30.
...
## 29. В документации, исходниках и EXE указаны разные версии

Сначала убедиться, что открыты документы и запускается EXE из одной копии
инструмента. Разные ветки, отдельные clones/worktrees и старые `bin` могут иметь
разные версии. Номера в CHANGELOG и формулировки «начиная с версии…» являются
историей, а не указанием текущей версии. Текущая версия исходников задана в
`Program.cs` (`ProductVersion`) и отражена в README этой же ветки.

Из каталога нужного Git checkout проверить:
...
Если исходники обновились, а EXE остался прежним, явно пересобрать инструмент:
...
`run.cmd`/`run.ps1` автоматически собирают инструмент только при отсутствии EXE;
существующий файл они не проверяют на актуальность относительно исходников.
В portable-копии без `.git` пропустить Git-команды. Сборка и `version` не
подключаются к TIA Portal и не изменяют проект или PLC. После обновления EXE
старый check-bundle не использовать для apply/sync: нужен свежий `check-clone`.

## 30. После recovery остаётся история ошибок или не обновляется completion

Сочетание `state=committed`, `diagnosticStatus=recovered` и непустого
`diagnosticDetails` допустимо: текущая попытка восстановления успешна, история
ранних ошибок сохранена. Сам completion — диагностический отчёт, а не
доказательство commit вместо журнала и проверки установленного состояния.

Если команда уже подтвердила commit и сообщает только ошибку completion/report,
не повторять `apply-clone --apply --save` и не восстанавливать TIA-проект лишь
ради записи отчёта. Сначала сохранить ошибку, журнал, completion, backup и пакет.
При временной файловой блокировке освободить handle после выяснения его владельца;
следующая clone-команда повторит обязательное recovery до начала новой работы.

При сообщении о чужом owner/transaction/workspace, неподдерживаемой схеме или
повреждённом JSON нужен ручной разбор привязок и сохранённых копий. Не удалять
журнал, staging или `_rollback` и не менять поля схемы, чтобы обойти отказ.
Committed recovery сохраняет journal/package и блокирует новую команду, пока
диагностический конфликт не разрешён. Если не пройдена проверка самого
установленного baseline, исход остаётся неразрешённым; completion с надписью
`committed` не отменяет этот отказ.

## 31. WORKSPACE_PATH_TOO_LONG_OR_INVALID

В `apply-clone-gate.csv` gate `workspace-path-budget` имеет `failed`. Он проверяет
будущие пути при непустом плане и блокирует apply до project backup/TIA-записи;
проверка выполняется также в dry-run. Даже короткий исходный файл может получить
слишком длинный путь во вложенном validation/sync/publication-каталоге.

Полный абсолютный путь файла должен быть короче 260, каталога — короче 248 единиц
UTF-16. Это не лимит только длины `--out` и не обещание неограниченных long paths.
Включение Windows long paths не отменяет этот контракт инструмента. Пустой
dry-run не проверяет бюджет будущей непустой записи.

Порядок действий:

1. Сохранить `apply-clone-gate.csv`, `apply-clone-summary.txt` и исходную ошибку.
   В gate посмотреть точный путь и причину: `OR_INVALID` может обозначать
   некорректный путь, а не только превышение длины.
2. Проверить, что отказ относится к текущему before-write gate, а не к прежней
   ошибке после записи. Если ранее была запись или остался неразрешённый журнал,
   сначала сверить состояние TIA и recovery-evidence; не повторять apply вслепую.
3. Для workspace без незавершённых транзакций выбрать более короткое размещение,
   сохранив локальные правки и evidence. Не перемещать незавершённую транзакцию:
   её привязки требуют исходных путей. Не удалять staging/journal/backup ради
   обхода отказа и не редактировать policy или path в старом bundle.
4. На новом пути выполнить свежие `check-clone` и dry-run. Проверить gate и весь
   план перед отдельным решением о реальной записи. Если причина относится к
   другому корневому каталогу, ориентироваться на путь из диагностики, а не
   считать сокращение `--out` универсальным исправлением.

Текущая write policy — `clone-write-policy-v13`; bundles от v12 не разрешают
применение новой сборкой. См. [ограничения путей](README.md#workspace-path-limits).
