...
исходникам этой ветки; фактическую версию выбранного EXE показывает `version`.
...
Write-команды TIA-проекта в текущей версии:
...
Publication journal schema 5 строго связывает canonical workspace, operation,
...
для вложенного sync внутри apply validation. Во время rollback активный компонент
перемещается по идентифицированному Windows handle в постоянный `_rollback`
внутри backup транзакции, вне удаляемого staging. После захвата проверяются ID и
fingerprint самого перемещённого объекта; поздняя правка останавливает recovery
с сохранением обеих версий. Лишь после успешной проверки возвращается старый
backup. Захваченные данные не удаляются автоматически даже после успешного
rollback: редактор может записать изменения через ранее открытый handle.
...
допишет completion и только затем освободит journal/package. Ошибка flush после
атомарной записи committed-журнала сама по себе не означает rollback: исход
проверяется по журналу на диске и установленным компонентам. Нечитаемый журнал
или несовпадение компонентов оставляют исход неразрешённым, без изменения данных
и без заявления об успешном commit/rollback.

Completion schema `2` содержит краткий `message` и полный `diagnosticDetails`,
включая вложенные причины, соседние ветви AggregateException и доступные stack
traces. История сохраняется и при recovery новым процессом без прежних объектов
исключений. `committed/recovered` с непустым `diagnosticDetails` означает успешную
текущую попытку восстановления с сохранённой историей ошибок.

Перед наследованием истории проверяются owner, схема, точный набор полей,
transactionId, operation и привязки workspace/backup. Чужой, повреждённый или
недоступный completion не перезаписывается молча: committed recovery сохраняет
journal/package, а вход новой команды блокируется до разрешения конфликта.
